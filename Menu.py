...
while running:
    ...
    if state == MENU:
        ...
        for num in range(3, 0, -1):
            screen.blit(background_map1, (0, 0))  # Redraw map background
            countdown_text = menu_font.render(str(num), True, (255, 255, 255))
            countdown_rect = countdown_text.get_rect(center=(screen_width // 2, 50))
            screen.blit(countdown_text, countdown_rect)
            pygame.display.flip()
            pygame.time.wait(1000)  # Wait for 1 second

        # After countdown finishes, show Wave 1 for 1 second
        screen.blit(background_map1, (0, 0))
        draw_wave(1)
        pygame.display.flip()
        ...
        screen.blit(background_map1, (0, 0))
        draw_wave(1)
        pause_rect = draw_pause_button()  # Draw the pause button

        # Check if the pause button is clicked
        if mouse_click and pause_rect.collidepoint(mouse_pos):
            previous_state = state
            state = PAUSE  # Go to PAUSE state if the pause button is clicked
        ...
        if mouse_click:
            if resume_rect.collidepoint(mouse_pos):
                state = previous_state  # Go back to the previous state
